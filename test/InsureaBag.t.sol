...
contract InsureaBagTest is PRBTest, StdCheats {
    ...
    function testTransferSampleNFTtoRandom() public {
        vm.prank(address(10));
        sampleNFT.safeMint(address(10), 1);
        assertEq(sampleNFT.ownerOf(1), address(10));

        vm.prank(address(10));
        sampleNFT.safeTransferFrom(address(10), address(20), 1);
        assertEq(sampleNFT.ownerOf(1), address(20));
    }

    function testERC721tokenTransferDirectlytoNFTContract() public {
        vm.prank(admin);
    ...
    function testERC721tokenTransferthroughUs() public {
        vm.prank(admin);
        nftContract.setRegistryAddress(address(registry));
    ...
    function testERC721tokenTransferDirectlytoNFTContractShouldBeSuccessfull() public {
        vm.prank(admin);
        nftContract.setRegistryAddress(address(registry));

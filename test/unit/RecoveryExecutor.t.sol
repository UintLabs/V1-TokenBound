...
contract TokenshieldSafe7579Test is BaseSetup {
    ...
    function getRecoveryHash(address _account, address _newOwner, uint64 _recoveryEndTime) public pure returns (bytes32) {
        return keccak256(
            abi.encode(
    ...
    function getRecoveryHashWithDomainSeperator(
        ...
        returns (bytes32)
    {
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", recoveryDomainSeparator(verifyingContract), transactionHash));
        return digest;
    }
